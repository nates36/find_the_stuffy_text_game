...
def find_stuffy():
    ...
    attempts = 3
    valid_items = [item.lower() for item in room_items]
    while attempts > 0:
        guess = input("Where do you think the stuffy is hidden? ").strip().lower()
        if not guess.lower():
            print("You entered nothing. Please type an item from the list.")
            continue
        if guess.lower() not in valid_items:
            print("That's not a valid item. Please choose from the list above.")
            continue
        if guess.lower() == stuffy_location.lower():
